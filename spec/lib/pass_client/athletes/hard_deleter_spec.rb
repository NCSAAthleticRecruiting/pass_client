...
require "pass_client/athletes/hard_deleter"

RSpec.describe PassClient::Athlete::HardDeleter do
  ...
  it "sends a request to the correct address" do
    expect(connection_double)
      .to receive(method).with(url: "/api/partner_athlete_search/v1/athlete/#{id}?hard_delete=true", headers: { authorization: token })
    subject.delete!
  end
